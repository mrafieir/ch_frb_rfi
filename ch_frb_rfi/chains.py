...
def detrender_chain(ix, detrend_nt=2048, cpp=True, plot_nt=16):
    """If plot_nt=0, then plots will not be generated."""

    transform_chain = [ rf_pipelines.polynomial_detrender(deg=4, axis=1, nt_chunk=detrend_nt, cpp=cpp),
                        rf_pipelines.polynomial_detrender(deg=8, axis=0, nt_chunk=detrend_nt, cpp=cpp) ]

    if plot_nt > 0:
        transform_chain += [ rf_pipelines.plotter_transform('dc_out%d' % ix , img_nfreq=512, img_nt=1200, downsample_nt=plot_nt) ]
...
def transform_chain(detrender_niter=2, clipper_niter=3, detrend_nt=2048, clip_nt=1024, two_pass=True, cpp=True, plot_nt=16):
    """If plot_nt=0, then plots will not be generated."""

    transform_chain = [ ]
    
    if plot_nt > 0:
        transform_chain += [ rf_pipelines.plotter_transform('raw', img_nfreq=512, img_nt=1200, downsample_nt=plot_nt) ]
    ...
    for ix in xrange(detrender_niter):
        ...
        transform_chain += detrender_chain(ix, detrend_nt=detrend_nt, cpp=cpp, plot_nt=plot_nt)
